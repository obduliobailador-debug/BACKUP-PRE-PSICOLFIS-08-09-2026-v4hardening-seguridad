...
// Enlaces de Stripe por agente y nivel (demo limitada / acceso completo).
// Se leen desde variables de entorno para poder cambiarlos sin redeploy.
...
const ADMIN_TOKEN_KEY = "psicolfis_admin_token";
  ...
  return (
    <div className="admin-shell" data-testid="admin-shell">
      ...
      <main className="admin-main">
        {tab === "budgets"
          ? <AdminBudgets token={token} onAuthFail={handleLogout} />
          : <AdminReviews token={token} onAuthFail={handleLogout} />}
      </main>
    </div>
  );
};
...
const useAdminApi = (token, onAuthFail) => ({
  get: async (path) => {
    try {
...
const AdminBudgets = ({ token, onAuthFail }) => {
...
const AdminReviews = ({ token, onAuthFail }) => {
  const api = useAdminApi(token, onAuthFail);
